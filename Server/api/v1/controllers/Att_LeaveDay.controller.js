...
module.exports.create = async (req, res) => {
  try{
    ...
    const result = await Att_LeaveDayModel.create({...data,DayLeave:DayLeave,ProfileID:Profile.ProfileID,Status:"CHUA_DUYET"});
...
module.exports.update = async (req, res) => {
  try{
    const { ID } = req.params;
    const  data  = req.body;
    const result = await Att_LeaveDayModel.findOneAndUpdate({_id:ID}, {...data,Status:"CHUA_TINH_CONG"},{new:true});
    const resultData=await Att_LeaveDayModel.aggregate([{
      $match:result
    },
    {
